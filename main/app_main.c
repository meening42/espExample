...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        esp_mqtt_client_subscribe(client, "example/temperature_request", 1);
        break;
    ...
    case MQTT_EVENT_DATA:
        ...
        char requestTopic[32] = "example/temperature_request";
        if(strncmp (event->topic,requestTopic,event->topic_len ) == 0){
            triggerSrc_t trigger = TRIGGER_MQTT_REQUEST;
            xQueueSendFromISR(evt_queue, &trigger, NULL);
        }else{
            printf("I gues not the same :(\n");
        }
        break;
...
static void myTask(void* arg)
{
    ...
    for (;;) {
        if (xQueueReceive(evt_queue, &src, portMAX_DELAY)) {
            ...
            switch(src){
                case TRIGGER_BTN:                   
                    snprintf(buffer, sizeof(buffer), "{\"trigger_src\":\"button\", \"temperature\":%.1f}\n", tsens_value);
                    break;
                case TRIGGER_TIMER:
                    snprintf(buffer, sizeof(buffer), "{\"trigger_src\":\"timer\", \"temperature\":%.1f}\n", tsens_value);
                    break;
                case TRIGGER_MQTT_REQUEST:   
                    snprintf(buffer, sizeof(buffer), "{\"trigger_src\":\"mqtt_request\", \"temperature\":%.1f}\n", tsens_value);
                    break;
                default:
                    snprintf(buffer, sizeof(buffer), "{\"trigger_src\":\"unknown\", \"temperature\":%.1f}\n", tsens_value);
                    break;
            }
            printf("%s",buffer);
            esp_mqtt_client_publish(client, "example/data", buffer, 0, 1, 1);
        }
    }
}
